...
if(isset($CEAction) && $CEAction == "register") {
	...
	if(empty($email) or empty($password) or empty($cpassword)) {
		...
		$last_inserted_id = get_last_inserted_id('ce_users');
		$wallet_id = generate_wallet_id($last_inserted_id);
		$db->query("UPDATE `ce_users` SET `wallet_id` = '$wallet_id' WHERE `id` = '$last_inserted_id'");
		$active_wallet_gateways = $db->query("SELECT * FROM `ce_wallet_gateways` WHERE `status` = 1 ORDER BY `id` ASC");
		while($wallet_gateway = $active_wallet_gateways->fetch_assoc())
		{
			$db->query("INSERT INTO `ce_user_wallets` (
				`uid` , `gateway_id` , `status`, amount
			) VALUES (
				'$last_inserted_id', ".$wallet_gateway['id'].", '1', '0'
			)");
		}
